 ...
 * 
 * This is a JavaScript version of the TypeScript metadata extractor
 * for use with CommonJS require in the test script
 */
 ...
 */
function findBestMoodMatch(title) {
  ...
  if (!bestMatch || bestMatch.score < 0.3) {
    const moodAssociations = {
      ...
      'Melancholic': ['sad', 'blue', 'alone', 'lost', 'tears', 'rain', 'miss', 'gone'],
  ...
  return bestMatch ? bestMatch.mood : undefined;
}

/**
 * Use fuzzy matching to find the best genre match for a title
 */
function findBestGenreMatch(title) {
 ...
 */
function getSimilarityScore(words, target) {
 ...
 */
function applyExtractedMetadata(track, extractedData) {
  const updated = { ...track };
 ...
 */
function getMetadataConfidence(metadata) {
  const confidence = {};
...
module.exports = {
  ...
  getAvailableMoods: () => AVAILABLE_MOODS,
  getAvailableGenres: () => GENRE_KEYWORDS
}; 